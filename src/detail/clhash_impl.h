...
namespace lemire {
    ...
    // hashing the bits in value using the keys key1 and key2 (only the first 64 bits of key2
    // are used). This is basically (a xor k1) * (b xor k2) mod p with length component
    ...
    uint64_t clhash(const void *random, const char *stringbyte, const size_t lengthbyte) {
        ...
        __m128i polyvalue = _mm_load_si128(
            rs64 + m128neededperblock); // to preserve alignment on cache lines for main loop,
                                        // we pick random bits at the end
