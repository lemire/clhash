...
TEST_CASE("Basic string tests", "") {
    ...
    CHECK(h1(short_string) == 15276029651472846521ULL);
    CHECK(h1(short_string.data(), short_string.size()) == 15276029651472846521ULL);
}

// TODO: Need to update this test.
TEST_CASE("std::vector") {
    util::CLHash hash_fcn;
    ...
    CHECK(hash_fcn(int_data2.data(), int_data2.size()) ==
          14240744024781019794UL); // Use util::CLHash
    CHECK(hash_fcn(int_data2.data(), int_data2.size()) ==
          hash_fcn(int_data2)); // Use util::CLHash
}

TEST_CASE("std::unordered_map") {
    auto dict = std::unordered_map<std::string, int, util::CLHash>();
